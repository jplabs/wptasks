...
function woo_load_frontend_css () {
	wp_register_style( 'theme-stylesheet', get_stylesheet_uri(), array(), '5.7.1', 'all' );
	...
	if ( isset( $woo_options['woo_style_disable'] ) && $woo_options['woo_style_disable'] != 'true' ) {} else {
		...
		if ( '' != $nav_margin_top  || '' != $nav_margin_bottom ) {
			if ( isset( $woo_options[ 'woo_header_full_width' ] ) && 'true' == $woo_options[ 'woo_header_full_width' ]  ) {
				...
				$output .= '#nav-container { margin-top:'.$nav_margin_top.'px;margin-bottom:'.$nav_margin_bottom.'px; }';
			} else {
...
function woo_add_full_width_class ( $classes ) {
	...
	if ( 'true' == $settings['header_full_width'] || 'true' == $settings['footer_full_width'] || ( 'true' == $settings['slider_biz_full'] && is_page_template( 'template-biz.php' ) ) ) {
		$classes[] = 'full-width';
